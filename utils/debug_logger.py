...
class DebugLogger:
    ...
    def _write_log(self, entry: Dict[str, Any]) -> None:
        """Write a log entry to file with proper JSON serialization"""
        entry['timestamp'] = datetime.now().isoformat()
        entry['session_id'] = self.session_id
        
        # Sanitize the entire entry
        sanitized_entry = self._sanitize_for_json(entry)
...
def get_debug_logger():
    """Get the singleton instance of DebugLogger"""
    global debug
    return debug

# Export the singleton instance
__all__ = ['debug', 'get_debug_logger']